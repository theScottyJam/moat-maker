...
describe('union rules', () => {
  ...
  test('if you match the required fields of one union variant, you can not set incorrect required fields of another.', () => {
  ...
  test('if you match the required fields of one union variant, you can not set incorrect optional fields of another.', () => {
    const v = validator`{ type: 'A', value?: 1 } | { type: 'B' }`;
    const act = (): any => v.assertMatches({ type: 'B', value: 2 });
    assert.throws(act, {
      message: 'Expected <receivedValue>.value to be 1 but got 2.',
  ...
  test('all union variants have a same required field', () => {
    ...
    const act = (): any => v.assertMatches({});
    // TODO: It would be better to realize that `x` is a common field in both variants, and to simply
    // throw an error saying that "x" is required.
    assert.throws(act, {
      message: [
        'Failed to match against any variant of a union.',
